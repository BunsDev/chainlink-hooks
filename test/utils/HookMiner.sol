...
library HookMiner {
    ...
    // maximum # iterations to find a salt; avoids infinite loops
    uint256 constant MAX_LOOP = 100_000;

    /// @notice Find a salt that produces a hook address with the desired `flags`
    /// @param deployer address that will deploy the hook. In `forge test`, this will be the test contract `address(this)` or the pranking address
    ///                 In `forge script`, this should be `0x4e59b44847b379578588920cA78FbF26c0B4956C` (CREATE2 Deployer Proxy)
    /// @param flags desired flags for the hook address
    /// @param creationCode creation code of a hook contract. Example: `type(SwapHook).creationCode`
    /// @param constructorArgs encoded constructor arguments of a hook contract. Example: `abi.encode(address(manager))`
    ...
    /// @param deployer address that will deploy the hook. In `forge test`, this will be the test contract `address(this)` or the pranking address
    ///                 In `forge script`, this should be `0x4e59b44847b379578588920cA78FbF26c0B4956C` (CREATE2 Deployer Proxy)
    /// @param salt salt used to deploy the hook
    /// @param creationCode creation code of a hook contract
